...
| **T — Tampering** | Malicious upload (PE, ELF, shell, PHP) reaches a converter. | Magic-byte deny-list rejects the request before any decoder runs; HTTP 415 returned. | `app/api/routes/convert.py`, `compress.py` (BLOCKED_MAGIC) | None known for the listed prefixes. |
...
| **T — Tampering** | SSRF via WeasyPrint URL fetching. | `url_fetcher=_deny_url_fetcher` denies every external fetch in WeasyPrint. | `app/main.py::_deny_url_fetcher`, PT-008 anchor | Operator must not patch this out — documented as a do-not-disable hardening item. |
